...
function showFrase() {
  ...
  fraseElem.classList.remove("hidden");
  versiculoElem.classList.add("hidden");
...
// Mostrar animación de respiración, frase y botón
function showClosing() {
  const closing = document.getElementById("closingContainer");
  const marca = document.getElementById("marcaFinal");
  const boton = document.getElementById("restartBtn");

  closing.classList.remove("hidden");

  // mostrar marca del proyecto y botón después de 8s
  setTimeout(() => {
